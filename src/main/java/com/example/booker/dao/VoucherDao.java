...
public interface VoucherDao extends JpaRepository<Voucher, Integer> {
    ...
    @Query("SELECT v FROM Voucher v JOIN CuaHang c on c.ma_cua_hang = v.ma_cua_hang WHERE c.ma_cua_hang = ?1 ORDER BY v.ma_voucher desc")
    ...
    @Query("SELECT COUNT(v) FROM Voucher v JOIN CuaHang c ON c.ma_cua_hang = v.ma_cua_hang WHERE c.ma_cua_hang = :maCuaHang")
    long countVouchersByCuaHang(@Param("maCuaHang") int maCuaHang);

    // Hàm đếm số luong voucher theo trạng thái
    @Query("SELECT COUNT(v) FROM Voucher v JOIN CuaHang c ON c.ma_cua_hang = v.ma_cua_hang WHERE c.ma_cua_hang = :maCuaHang AND v.trangThai = :trangThai")
    ...
    @Query("SELECT v FROM Voucher v JOIN CuaHang c ON c.ma_cua_hang = v.ma_cua_hang WHERE c.ma_cua_hang = :maCuaHang AND v.trangThai = :trangThai")
...
//    tìm kiếm voucher theo tên
    @Query("SELECT v FROM Voucher v " +
            "JOIN CuaHang c ON c.ma_cua_hang = v.ma_cua_hang " +
